...
Then, in <span class="caps">Part II</span>, with truth tables under our belts, we look at modal concepts, starting with the modal logic S5, since this is a natural extra step to take. Then from there we explore S4 and multimodal S5 (as a gesture toward introducing accessibility relations), but then spend time on actuality, & double-indexed frames for two-dimensional modal logic, since this is rich in philosophical applications.

Once we do that, we turn to the question of natural deduction for modal logic, which introduces the idea of a side condition in a natural deduction proof. (How do you prove that *A* is necessary? Prove *A*, but only from *modal* assumptions.)
